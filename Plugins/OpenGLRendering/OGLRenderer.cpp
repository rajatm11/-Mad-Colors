...
OGLRenderer::OGLRenderer(Window& w) : RendererBase(w)	{
	...
	debugLinesMesh->SetVertexPositions(std::vector<Vector3>(10000, Vector3()));
	debugLinesMesh->SetVertexColours(std::vector<Vector4>(10000, Vector3()));
...
void OGLRenderer::RenderFrame(int curFrame)		{
...
void OGLRenderer::BindShader(ShaderBase*s) {
	if (!s) {
		...
		string msg = __FUNCTION__ " has received invalid shader?!";
...
void OGLRenderer::BindMesh(MeshGeometry*m) {
	if (!m) {
		...
		if (oglMesh->GetVAO() == 0) {
			string msg = __FUNCTION__ " has received invalid mesh?!";
			Debug::PrintToConsole(msg);
		}
...
void OGLRenderer::DrawBoundMesh(int subLayer, int numInstances) {
	if (!boundMesh) {
		string msg = __FUNCTION__ " has been called without a bound mesh!";
		Debug::PrintToConsole(msg);
		return;
	}
	if (!boundShader) {
		 string msg = __FUNCTION__ " has been called without a bound shader!";
		 Debug::PrintToConsole(msg);
		return;
	}
	GLuint	mode	= 0;
...
// 8508
void OGLRenderer::BindTexturesToShader(unsigned int t, const std::string& uniform, int texUnit){
	if (!boundShader) {
		string msg = __FUNCTION__ " has been called without a bound shader!";
		Debug::PrintToConsole(msg);
		return;//Debug message time!
...
void OGLRenderer::InitWithWin32(Window& w) {
	...
	if (!(deviceContext = GetDC(realWindow->GetHandle()))) {
		string msg = __FUNCTION__ " Failed to create window!";
		Debug::PrintToConsole(msg);
		return;
	}
	...
	if (!(PixelFormat = ChoosePixelFormat(deviceContext, &pfd))) {	// Did Windows Find A Matching Pixel Format for our PFD?
		string msg = __FUNCTION__ " Failed to choose a pixel format!";
		Debug::PrintToConsole(msg);
		return;
	}

	if (!SetPixelFormat(deviceContext, PixelFormat, &pfd)) {		// Are We Able To Set The Pixel Format?
		string msg = __FUNCTION__ " Failed to set a pixel format!";
		Debug::PrintToConsole(msg);
		return;
	}
	...
	if (!(tempContext = wglCreateContext(deviceContext))) {	// Are We Able To get the temporary context?
		string msg = __FUNCTION__ " Cannot create a temporary context!";
		Debug::PrintToConsole(msg);
	...
	if (!wglMakeCurrent(deviceContext, tempContext)) {	// Try To Activate The Rendering Context
		string msg = __FUNCTION__ " Cannot set temporary context!";
		Debug::PrintToConsole(msg);
		wglDeleteContext(tempContext);
		return;
	}
	if (!gladLoadGL()) {
		string msg = __FUNCTION__ " Cannot initialise GLAD!";
		Debug::PrintToConsole(msg); //It's all gone wrong!
		return;
	}
	...
	if (major < 3) {					//Graphics hardware does not support OGL 4! Erk...
		string msg = __FUNCTION__ " Device does not support OpenGL 4.x!";
		Debug::PrintToConsole(msg);
		wglDeleteContext(tempContext);
		return;
	}
	...
	if (major == 4 && minor < 1) {	//Graphics hardware does not support ENOUGH of OGL 4! Erk...
		string msg = __FUNCTION__ " Device does not support OpenGL 4.1!";
		Debug::PrintToConsole(msg);
		wglDeleteContext(tempContext);
		return;
	}
	...
	if (!renderContext || !wglMakeCurrent(deviceContext, renderContext)) {
		string msg = __FUNCTION__ " Cannot set OpenGL 3 context!";	//It's all gone wrong!
		Debug::PrintToConsole(msg);
	...
	string msg = __FUNCTION__ " Initialised OpenGL " + std::to_string(major) + "." + std::to_string(minor) + " rendering context";	//It's all gone wrong!
	Debug::PrintToConsole(msg);
...
static void APIENTRY DebugCallback(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar *message, const void *userParam) {
	...
	string msg = "OpenGL Debug Output: " + sourceName + ", " + typeName + ", " + severityName + ", " + string(message);
	Debug::PrintToConsole(msg);
}
#endif