...
PhysicsSystem::PhysicsSystem(GameWorld& g) : gameWorld(g)	{
	...
	linearDamping	= 0.5f;
	SetGravity(Vector3(0.0f, -30.0f, 0.0f));
}
...
void PhysicsSystem::Update(float dt) {	
	if (Window::GetKeyboard()->KeyPressed(KeyboardKeys::B)) {
		...
		string msg = "Setting broadphase to " + useBroadPhase;
		Debug::PrintToConsole(msg);
	}
	if (Window::GetKeyboard()->KeyPressed(KeyboardKeys::I)) {
		constraintIterationCount--;
		string msg = "Setting constraint iterations to " + constraintIterationCount;
		Debug::PrintToConsole(msg);
	}
	...
	applyGravity = Debug::GetApplyGravity();
	...
	if (!warmStart) {
		...
		if (updateTime > realDT) {
			realHZ /= 2;
			realDT *= 2;
			string msg = "Dropping iteration count due to long physics time...(now " + std::to_string(realHZ) + ")\n";
			Debug::PrintToConsole(msg);
		}
			...
			if (temp != realHZ) {
				string msg = "Raising iteration count due to short physics time...(now " + std::to_string(realHZ) + ")\n";
				Debug::PrintToConsole(msg);
			}
		}
	}
...
void PhysicsSystem::UpdateCollisionList() {
	for (std::set<CollisionDetection::CollisionInfo>::iterator i = allCollisions.begin(); i != allCollisions.end(); ) {
		if (i->a->ToRemove() || i->b->ToRemove()) {
			i = allCollisions.erase(i);
		}
		else {
			if ((*i).framesLeft == numCollisionFrames) {
				i->a->OnCollisionBegin(i->b, i->point);
				i->b->OnCollisionBegin(i->a, i->point);
			}
...
void PhysicsSystem::BroadPhase() {
	...
	Debug::SetNumBroadphaseCollisions(broadphaseCollisions.size());
}
...
void PhysicsSystem::NarrowPhase() {
	...
	Debug::SetNumNarrowphaseCollisions(allCollisions.size());
}
