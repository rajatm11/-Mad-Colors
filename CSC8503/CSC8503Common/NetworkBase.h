...
struct PlayerIDPacket : public GamePacket {
	int		playerID = -1;
	int		objectID = -1;
	PlayerIDPacket(int pid, int oid) {
		type = Player_ID;
		size = sizeof(PlayerIDPacket);
		playerID = pid;
		objectID = oid;
	}
};

struct ClientStartPacket : public GamePacket {
	ClientStartPacket() {
		type = Client_Start;
		size = sizeof(ClientStartPacket);
	}
};

struct DisconnectConfirmPacket : public GamePacket {
	int		playerID = -1;
	DisconnectConfirmPacket(int pid) {
		type = Disconnect_Confirmation;
		size = sizeof(DisconnectConfirmPacket);
		playerID = pid;
	}
};
