...
MeshGeometry::MeshGeometry(const std::string&filename) {
	...
	if (filetype != "MeshGeometry") {
		std::string msg = "File is not a MeshGeometry file!";
	...
	if (fileVersion != 1) {
		string msg = "MeshGeometry file has incompatible version!";
		Debug::PrintToConsole(msg);
		return;
	}
...
bool MeshGeometry::ValidateMeshData() {
	if (GetPositionData().empty()) {
		string msg = __FUNCTION__ " mesh " + debugName + " does not have any vertex positions!";
		Debug::PrintToConsole(msg);
		return false;
	}
	if (!GetTextureCoordData().empty() && GetTextureCoordData().size() != GetVertexCount()) {
		string msg = __FUNCTION__ " mesh " + debugName + " has an incorrect texture coordinate attribute count!";
		Debug::PrintToConsole(msg);
		return false;
	}
	if (!GetColourData().empty() && GetColourData().size() != GetVertexCount()) {
		string msg = __FUNCTION__ " mesh " + debugName + " has an incorrect colour attribute count!";
		Debug::PrintToConsole(msg);
		return false;
	}
	if (!GetNormalData().empty() && GetNormalData().size() != GetVertexCount()) {
		string msg = __FUNCTION__ " mesh " + debugName + " has an incorrect normal attribute count!";
		Debug::PrintToConsole(msg);
		return false;
	}
	if (!GetTangentData().empty() && GetTangentData().size() != GetVertexCount()) {
		string msg = __FUNCTION__ " mesh " + debugName + " has an incorrect tangent attribute count!";
		Debug::PrintToConsole(msg);
		return false;
	}

	if (!GetSkinWeightData().empty() && GetSkinWeightData().size() != GetVertexCount()) {
		string msg = __FUNCTION__ " mesh " + debugName + " has an incorrect skin weight attribute count!";
		Debug::PrintToConsole(msg);
		return false;
	}

	if (!GetSkinIndexData().empty() && GetSkinIndexData().size() != GetVertexCount()) {
		string msg = __FUNCTION__ " mesh " + debugName + " has an incorrect skin index attribute count!";
		Debug::PrintToConsole(msg);
		return false;
	}
