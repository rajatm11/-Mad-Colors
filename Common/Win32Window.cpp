...
Win32Window::Win32Window(const std::string& title, int sizeX, int sizeY, bool fullScreen, int offsetX, int offsetY) {
	...
	if (!GetClassInfoEx(windowInstance, WINDOWCLASS, &windowClass)) {
		...
		if (!RegisterClassEx(&windowClass)) {
			Debug::PrintToConsole("Window::Window(): Failed to register class!");
			return;
		}
	}
...
void	Win32Window::SetFullScreen(bool fullScreen) {//imgui here
...
void Win32Window::CheckMessages(MSG& msg) {
	...
	case (WM_INPUT): {
		...
		ExInputResult exInputRes = { false, false };//imgui here
		if (thisWindow->exInputFunc)//imgui here
			exInputRes = thisWindow->exInputFunc(raw);//imgui here
